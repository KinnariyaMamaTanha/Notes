...
1. switch语句中的每一个case都只能处理整型（包括`char`类型，枚举类型），不能处理浮点型
2. 常量表达式中不能包含变量
...
switch (expression) 
{
	case (constexpression1): statement1; break;
	case (constexpression2): statement2; break;
	……
	default: statement(n+1);
}
```
>[!warning]
>在遇到一个case的条件满足后，会依次执行之后所有case后的statement，直到遇到break或者default
