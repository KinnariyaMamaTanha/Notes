...
将结点插入进红黑树时，也会成为叶结点，并且其颜色不能是黑色（否则不满足第4条性质），因此**只能是红色的**。当其父结点为黑色时，直接插入即可；但当其父结点为红色时，又会违反第3条性质，此时必须执行旋转和着色，共有2种情况：

1. 父结点的兄弟结点S为黑色的（当不存在时，默认为黑色）
