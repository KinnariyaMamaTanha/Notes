...
在循环队列中，不好判断队列是否为空或是否为满，因为这两种情况下，均有front和rear指向同一个单元
此时可以选择牺牲一个单元，使front指向的单元不储存元素，则当rear绕一圈追上front时，队列就是满的，即`(rear + 1) % maxSize == front`
当队头追上队尾时，队列是空的，即`rear == front`
![[循环队列判空.png]]
