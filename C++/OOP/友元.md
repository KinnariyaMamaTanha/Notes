...
1. 格式：将原型放在类声明中，并加上关键字friend；编写定义时，不能用关键字`friend`与类限定符`className::`
```c++
// Time.h
class Time
{
...
当然也可以用以下方式来解决[[运算符重载#^7a1a5c|运算符重载中的问题]]
```c++
Time operator*(int m, const Time & t)
{
	return t * m;
...
1. 友元函数不在类作用域内，所以使用private数据成员时需要使用[[C++/OOP/类#^3688a6|限定名]]，而不是非限定名，实际上在任何类作用域外的地方都应该用限定名
...
```
友元声明可用于原始类的公有、私有或保护部分，此时友元类的所有方法可访问原始类的所有私有成员和保护成员

2. 使类中某些特定的成员函数成为原始类的友元
```c++
class class1;
class class2
{
...
class class1
{
	public:
		...
		// func被设置为class1的友元函数，拥有了class1的类作用域访问权限
}
```
...
3. 使用向前声明后，在读取到`class1`的声明后，编译器已经知道了`class1`是一个类，因此若需要将`class1`中的成员函数仍然定义为内联的，需要使用`inline`关键字（任意在类声明中或成员函数定义前一处使用即可），又由于**内联函数的链接性是内部的**，所以此时必须在类声明的头文件中定义
