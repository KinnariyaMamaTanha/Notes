...
2. 类模板不是类，仅仅是C++编译器指令，用于说明如何生成类的定义；模板的具体实现称为**实例化**或**具体化**
...
2. 可以在`template <>`的尖括号中写入`int n`等，这时`n`被称为**非类型参数**（**表达式参数**），可以用于模板类中
	1. 另外，表达式参数只能是**整型、枚举、引用或指针**
	2. 模板代码不能修改表达式参数的值，也不能使用其地址
	3. 作为表达式参数的值必须是[[分支#^22c911|常量表达式]]
...
>[!note]
>*和[[C++/Grammars/Function/函数#^f96caf|函数模板的具体化]]类似，隐式实例化、显式实例化、显式具体化统称具体化*
